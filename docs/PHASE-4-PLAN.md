...
- **Cron-friendly.** A second Python entrypoint (`setlist-stash-resolve`)
...
A separate process (`setlist-stash-resolve`) runs every 30 minutes via either:

- **Option A:** docker compose `cron` profile + a host-level `cron` entry
  invoking `docker compose --profile cron run --rm setlist-stash-resolve`.
...
A small async wrapper (`src/setlist_stash/mcp_client.py`, **stub for kickoff**)
...
1. **Container health.** `docker compose ps` shows `setlist-stash` healthy
   and `setlist-stash-pg` healthy on nix1; `curl http://nix1:3706/healthz`
   returns 200 with `{"status":"ok","version":"<v>"}`.
2. **Schema migrated.** `setlist-stash-pg` has `schema_version` row 1, all
   five tables exist, `predictions_lock_guard` trigger exists.
3. **MCP path works.** A curl from inside the `setlist-stash` container to
...
6. **Resolver works.** A scripted test: insert a fake `prediction_locks`
   ...
   1995-12-30 with known setlist). Run `setlist-stash-resolve`. Confirm:
...
10. **Memory updated.** `phish-platform.md` shows Phase 4 status
    advanced; `homelab-webapps.md` claims port 3706 as `setlist-stash`.
...
| 5 | Resolver entrypoint + scoring formula + scoring_runs audit | `setlist-stash-resolve` CLI |
