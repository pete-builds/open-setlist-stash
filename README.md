...
- **Show-day pick reminders** are gated on `REMINDER_ENABLED` (default off) and require a working `EMAIL_PROVIDER`. When on, the `setlist-stash-reminders` service wakes at `REMINDER_HOUR_LOCAL`:`REMINDER_MINUTE_LOCAL` in `DISPLAY_TZ` (default 09:00 Eastern) and, **only on days a show is scheduled**, mails players who have a verified email, have not unsubscribed, and have not submitted picks yet. One message per player per show, enforced by a unique row in `email_sends` so a container restart cannot double-send. Every message carries a signed unsubscribe link plus RFC 8058 `List-Unsubscribe` / `List-Unsubscribe-Post` headers, and players can also toggle reminders from `/account`. Nothing is sent within `REMINDER_MIN_LEAD_MINUTES` of the pick cutoff. Unsubscribing never affects sign-in mail.
